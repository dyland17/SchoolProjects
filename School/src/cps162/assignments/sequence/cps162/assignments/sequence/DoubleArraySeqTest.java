...
public class DoubleArraySeqTest {
	...
	@Test
	public void testRemoveCurrentFullCapacity(){
		DoubleArraySeq array = new DoubleArraySeq(2);
		array.addBefore(25);
		array.addAfter(50);
		array.removeCurrent();
		assertEquals("No current Index", false, array.isCurrent());
	}
	...
	public void testEnsureCapacity(){
		...
		assertEquals("Nothing should happen because the capacity is greater than min.", 10, data.getCapacity());
		
		data.ensureCapacity(10);
		assertEquals("Nothing should happen because the capacity is equal to min.", 10, data.getCapacity());
		
		data.ensureCapacity(2);
		assertEquals("MinimumCapacity is smaller than size of data. So nothing should happen.", 10, data.getCapacity());
		
		data.ensureCapacity(7);
		assertEquals("The data array length should shrink to 7.", 7, data.getCapacity());
